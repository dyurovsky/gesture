...
demos <- read_feather("../../feathers/demos.feather") %>%
  filter(id != 47, id != 110) %>%
  mutate(income2=ordered(round(income), levels=c(1, 2, 3, 4, 5, 6), labels=c("Less than $15,000", "$15,000 to $34,999", "$35,000 to $49,999", "$50,000 to $74,999", "$75,000 to $99,999", "greater than $100,000")))
...
Although we coded for concrete referents that had the potential to be produced either in speech or in gesture, we found that these referents were not always physically present in the environment. Not every referent had a nonzero probability of being gestured to, as parents and children occasionally talked about events that have happened in the past or future, items that are in different locations, or characters who are not present. Therefore, we coded for whether the referent was present in the environment, and included only those referents that were present in the following analysis. On average, `r round(mean(presentness$ref_predicted),2)*100`% of referents were present during the discourse. The predicted number of referents present ranged from `r round(min(presentness_age$mean),2)*100`% present at age=`r min(presentness_age$age)`mos to `r round(max(presentness_age$mean),2)*100`% present at age=`r max(presentness_age$age)`mos. Referent presentness did not appear to vary based on whether it was produced by the parent or child. 
...
In order to assess the reliability of concrete noun coding, agreement was calculated for two independent coders. The Krippendorff's Alpha statistic for a comparison of 27% of the data was $\alpha$  = `r round(irr_coef$value,2)`. Issues and discrepancies in coding decisions were dicussed and resolved during the formation of a coding manual.  
