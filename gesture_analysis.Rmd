...
sample_ref <- split_df(ref, 16, 20) %>%
  ...
  select(time, subj, age, sample, person, referent, modality, chat, gloss) 

sample_2_ref<-sample_ref %>%
  group_by(subj, age, sample, person)  %>%
  summarize(count=sum(is.na(referent)==FALSE)) %>%
  group_by(subj, age, person) %>%
  mutate(avg_ref = sum(count) / 20) 


#shows number of referents across samples for each age
ggplot(aes(x = age, y=count), data=sample_2_ref) + 
  geom_bar(stat="identity")+ ylab("average referents per discourse")

#shows average number of referents within samples--interesting that for older children, there are not more referents (maybe going more in depth on one? or more pronoun usage?)
sample2_ref <- sample_2_ref %>%
  filter(sample==1) %>%
  group_by(person, age) %>%
  mutate(avg_age_ref = sum(avg_ref) / 10) %>%
  filter(subj==29) 

ggplot(aes(x = age, y=avg_age_ref, fill=person), data=sample2_ref) + 
  geom_bar(stat="identity", position="dodge")+ ylab("average referents per discourse") + theme(legend.position="bottom")

#shows general plot of one sample instance of discourse, just to see what it looks like 
...
#now want to see how many referents are first referents? ***WORK IN PROGRESS*** 
first_data <- coded_responses %>%
  group_by(subj, age, referent) %>%
  slice(1) %>% 
  mutate(first = TRUE) 

first_times <- first_data %>%
  select(subj, age, time)

first_ref <- sample_ref %>%
  group_by(subj, age) %>%
  filter(time %in% first_times$time)
...
#how long are these instances (repetitions)? How do we capture just one instance of repeated mom? 
mom_repeat<-matrix(nrow=nrow(coded_responses), ncol=ncol(coded_responses))

for(i in 1: nrow(coded_responses)) {
    if(repeated=referent="mom") {
