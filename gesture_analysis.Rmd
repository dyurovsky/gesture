...
transition_responses <- coded_responses %>%
  ...
  mutate(last_person = lag(person), last_referent = lag(referent)) %>%
  unite(type, person, modality, remove = FALSE) %>%
  mutate(last_type = lag(type)) %>%
  filter(last_person != person & referent == last_referent) %>%
  filter(!is.na(last_type)) %>%
  group_by(subj, age, person, referent) %>%
  group_by(subj, age, person, referent, last_type, type) %>%
  summarise(n = n()) %>%
  mutate(n = n/sum(n)) %>%
  group_by(last_type, type, referent) %>%
  summarise(n = mean(n)) %>%
  group_by(last_type, type)

  #what we are trying to do here ^^^^ is to see the transition probability not between all modalities/people (we saw), but to see transition probability from parent/child (different person) if the referent is the SAME--let's look at the difference between those  


ggplot(transition_responses, aes(x = last_type, y=type, fill = n)) + 
  geom_tile() + 
  theme(legend.position = "right")

transition_responses_2<-coded_responses %>%
...
ggplot(transition_responses_2, aes(x = age, y = mean, color = person,
                                 label = person)) + 
...
trans.matrix()
>>>>>>> 3737dc2949af87339c068f31b7846c8b053faa9c
```
