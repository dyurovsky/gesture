...
loaded_subjs <- map(files, fread, sep=",", na.strings = c("NA", "")) %>%
  ...
  group_by(subj, age) %>%
  mutate(time = 1:n()) %>%
  select(time, subj, age, person, spoken_obj, gestured_obj, referent, gloss) 

#this data set does not include give or take gestures
no_give_subjs <- loaded_subjs %>%
  mutate(give = str_detect(gloss, "give"),
         take = str_detect(gloss, "take")) %>%
  filter(!give, !take)
...
datatable(abc_referents)

#edit the data to fix any referent errors 
#####
#
...
#subset of only child referents
child_referents <- loaded_subjs %>%
  filter(person=="child") %>%
  ungroup() %>%
  select(referent) %>%
  filter(!is.na(referent)) %>%
...
datatable(child_referents, rownames=FALSE, width=600)
count (child_referents) #571 unique child referents

parent_referents <- loaded_subjs %>%
...
?arrange
#order(referent)
datatable(parent_referents, rownames=FALSE, width=600)
count(parent_referents) #1008 unique parent referents

#it is interesting that there are parent referents that are not child referents, what are these? 
#unique_child_referents<- loaded_subjs %>%
 # filter(person)

#maybe make a printout of referent that shows whether it was child or adult or both who said it!! how do this

#compare the data sets and delete the referents that are the same between, include only those that are different
