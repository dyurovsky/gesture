...
Here is a gleitman plot that is actually interpretable, for giving an example of what a discourse might look like in the data:

```{r gleitman_readable, fig.width = 8, fig.height = 4}
discourse_2<-coded_responses %>%
  ...
  group_by(subj, age, referent, time) %>%
  slice(1) %>%
  filter(subj==33, age==22, time >500) #arbitrary choice

ggplot(discourse_2, aes(x = time, y = referent, fill = person)) + 
...
Does parent discourse responsiveness stay constant over age? Or is it larger for small children? 
...
```{r prop_people}
ggplot(discourse_prop, aes(x=age, y=prop, color=success, label = success)) + 
  facet_wrap(~person) + 
  geom_line() + 
  geom_point() + 
  theme(legend.position="bottom") + 
...
```{r repeat_data}
repeat_data <- discourse_success %>%
  filter(person == "child")%>%
  mutate(rep = ifelse(lag(success) == "repeat", "repeating", "not repeating")) %>%
  filter(is.na(rep)==FALSE) %>%
  group_by(rep, success) %>%
  summarise(n=n()) %>%
  mutate(prop = n/sum(n))

ggplot(aes(x=success, y=prop, fill = success), data=repeat_data)+
  geom_bar(stat="identity")+
  scale_fill_brewer(palette = "Set1") +
  facet_wrap(~rep)
```

They do seem to do a lot of repeating...
