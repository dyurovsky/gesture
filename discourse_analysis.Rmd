...
ggplot(discourse_prop2, aes(x=age, y=prop, color = success)) + 
  facet_wrap(~person) + 
  geom_point()+
  geom_line()+
  scale_color_brewer(palette = "Set1") +
...
ggplot(aes(x=success, fill=modality), data=discourse_success)+
  geom_bar(position="dodge")+
  theme(legend.position="bottom")+
  facet_grid(person~age)+
  ggtitle("age, person, modality, discourse type freq")+
  scale_color_brewer(palette = "Set1")

ggplot(aes(x=age, fill=modality), data=discourse_success)+
  geom_bar(position="dodge")+
  theme(legend.position="bottom")+
  facet_grid(person~success)+
  ggtitle("discourse type, person, modality, age freq")+
  scale_color_brewer(palette = "Set1")
...
ggplot(aes(x=success, y=prop, color=modality, label=modality), data=mod_freq)+
  geom_point()+
  geom_line(aes(group=modality))+
  theme(legend.position="bottom")+
  facet_grid(person~age)+
  ggtitle("age, person, modality, discourse type proportions")+
  scale_color_brewer(palette = "Set1")
```
...
repeat_data <- discourse_success %>%
  filter(person == "child")%>%
  mutate(rep = ifelse(lag(success) == "repeat" & success != "repeat", 1, 0)) %>%
  filter(is.na(rep)==FALSE) %>%
  group_by(rep, success) %>%
  summarise(n=n()) %>%
  mutate(prop = n/sum(n))

ggplot(aes(x=success, y=prop), data=repeat_data)+
