...
Unfortunately, this similarity data set only includes 350/1500 referents, so it is probably not the best computational comparison to use.
...
basic_words<-c("bag", "chair", "school", "circle", "car", "ground", "bus", "man", "tree", "square", "triangle", "pen", "button", "book", "floor", "water", "hole", "tree", "box", "shape", "top", "bottom")
...
people_words<-c("boy", "mom", "dad", "sister", "brother", "grandmother", "grandfather", "aunt", "uncle", "child", "teacher", "girl", "great_aunt", "zookeeper_wife", "cousin", "kid")
clothes_words<-c("belt", "shorts", "shirt", "hat", "glasses", "sunglasses", "sock", "slipper", "boot", "pants", "watch", "gown", "sweater", "dress", "necklace", "pocket", "hood", "sneaker", "shoe")
...
gleit<-fixed_runs %>%
  filter(subj==103, age==34)#arbitrary choice
...
topic_runs <- fixed_runs %>%
  filter(subj==33 | subj==103) %>%
  select(subj, age, referent, time, topic) %>%
  group_by(subj, age, referent) %>%
...
topic_gleit<-fixed_topic_runs %>%
  filter(subj==103)

topic_gleit_order <- topic_gleit %>%
  ungroup() %>%
  group_by(topic) %>%
  arrange(time) %>%
  slice(1) %>%
...
topic_gleit_ordered <- topic_gleit %>%
  mutate(referent = factor(topic, levels = topic_gleit_order$topic))
